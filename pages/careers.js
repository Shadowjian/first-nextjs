...
const Careers = ({ employers, numOfEmployers }) => {
  return (
    <Layout>
      <CareersPage employers={employers} numOfEmployers={numOfEmployers} />
    </Layout>
  )
}
...
export async function getServerSideProps(context) {
  try {
    ...
    const getEmployersDataResponse = await axios.get(
      `${baseUrl}/api/employer/employer-list?keyword=${keyword || ""}`
    )
    const getOpenEmployersDataResponse = await axios.get(
      `${baseUrl}/api/employer/number-of-open-employers`
    )
    ...
    return {
      props: {
        ...
        numOfEmployers: getOpenEmployersData.data.length || ""
      }
    }
  } catch (error) {
    console.log(error)
